...
router.post('/submit', async (req, res) => {
  const { user, items, province } = req.body;
  if (!province) return res.status(400).json({ msg: 'Province required' });
  const order = new Order({ user, items, province }); 
  await order.save();
  const recs = defaultFoodItems.filter(f => f.provinces.includes(province));
  res.json({ msg: 'Order placed!', recs });
});

// Payment stub for eSewa
router.post('/pay', (req, res) => {
  const { orderId, amount, esewaRef } = req.body;
  console.log(`Payment NPR ${amount} for order ${orderId}, ref ${esewaRef}`);
  res.json({ success: true, msg: 'Payment successful via eSewa' });
});
