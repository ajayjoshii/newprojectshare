...
//     try {
//       await axios.post("http://localhost:5000/api/users/upload-image", form, {
//         headers: {
//           "x-auth-token": localStorage.getItem("token"),  // must match backend auth header
...
//                      file:text-yellow-600 file:bg-yellow-50
...
export default function Profile({ user, onLogout }) {
  ...
  // Update local profileUser if `user` prop changes
  useEffect(() => {
    setProfileUser(user);
  }, [user]);
