...
// //         const res = await axios.get(`${BASE_URL}/api/orders/recommendations`, {
// //           params: { userId },
// //           headers: { Authorization: `Bearer ${token}` },
// //         });

// //         if (res.data.success) {
// //           setRecommendations(res.data.recommendations); // store recommended items
// //         }
// //       } catch (error) {
// //         console.error("Could not fetch recommended items.", error);
// //       }
// //     };
...
//   // ------------------ Detect geolocation if province not set ------------------
//   useEffect(() => {
//     if (province) return; // already set from profile
//     navigator.geolocation.getCurrentPosition(
//       ({ coords }) => {
//         const { latitude: lat, longitude: lng } = coords;
//         let prov = "Unknown";
...
//   // ------------------ Auto-set province to defaultFoodItems ------------------
//   const foodItemsWithProvince = defaultFoodItems.map(item => ({
//     ...item,
//     province: province || "Unknown"
//   }));

//   // ------------------ Recommendations based on province & user ------------------
//   useEffect(() => {
//     if (!province) return;

//     // Fetch user past orders if logged in
//     const fetchUserOrders = async () => {
//       if (!user) {
//         // User not logged in, fallback to province-based recommendation
//         const recs = foodItemsWithProvince.filter(item => item.province === province);
...
//         const pastItems = res.data.flatMap(order => order.items); // flatten all past order items
//         const pastItemIds = pastItems.map(i => i.id);

//         // Recommend items from past orders first (user-specific)
//         let userSpecificRecs = foodItemsWithProvince.filter(item =>
//           pastItemIds.includes(item.id)
//         );

//         // Fill remaining slots with province-based items
//         const remaining = foodItemsWithProvince.filter(
//           item => item.province === province && !userSpecificRecs.some(r => r.id === item.id)
//         );

//         setRecommendations([...userSpecificRecs, ...remaining].slice(0, 8));
//       } catch {
//         // fallback if fetching orders fails
//         const recs = foodItemsWithProvince.filter(item => item.province === province);
//         setRecommendations(recs.slice(0, 8));
//       }
//     };
